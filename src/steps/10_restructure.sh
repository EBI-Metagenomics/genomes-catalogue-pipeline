...
if [ -f "${OUT}"/IQtree/gtdbtk.ar53.user_msa.fasta ]; then
...
while read line; do
    if [ ! -f "${RESULTS}"/species_catalogue/"${line::-2}"/"${line}"/genome/"${line}"_sanntis.gff ]; then
        touch "${RESULTS}"/species_catalogue/"${line::-2}"/"${line}"/genome/"${line}"_sanntis.gff
    fi

    if [ ! -f "${RESULTS}"/species_catalogue/"${line::-2}"/"${line}"/genome/"${line}"_virify.gff ]; then
