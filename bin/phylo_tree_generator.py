...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Tool to generate a phylogenetic tree from a tsv file"
    )
    parser.add_argument(
        "--table",
        dest="tsv_file",
        help="2-column file containing the genome name and lineage",
    )
    parser.add_argument(
        "--out", dest="out", default="tree.json", help="Tree file output"
    )
...
json_root = {"children": [], "name": "Domain", "type": "root"}


def add_to_json(json_root, genome, lineage):
    if len(lineage) == 0:
        new_node = {"coungen": 1, "name": genome, "type": "genome"}
    ...
    new_node = {"countgen": 1, "name": l, "type": l[0], "children": []}
    json_root["children"].append(new_node)
