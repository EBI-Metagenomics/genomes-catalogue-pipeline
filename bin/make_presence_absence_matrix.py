...
def parse_clusters(adjacency_file):
    ...
    Gene IDs must follow the <sample_name>_<gene_number> convention so that sample
    identity can be recovered by splitting on the last underscore. Prokka satisfies
    this automatically when run with --locustag <sample_name>.
    ...
    -------
    clusters : dict
        {representative_id: {sample_name: [member_locus_tag, ...]}}
        Each key is a cluster representative; values map sample names to the list
        of member locus tags from that sample belonging to the cluster.
    all_samples : list[str]
        Sorted list of all sample names found across all clusters.
    """
    clusters = {}
    all_samples = set()
    ...
    for line in lines:
        ...
        sample = member.rsplit("_", 1)[0]
        all_samples.add(sample)
        clusters.setdefault(rep, {}).setdefault(sample, []).append(member)

    return clusters, sorted(all_samples)
...
def parse_gff_annotations(gff_files):
    ...
    comment lines, the ##FASTA directive, FASTA headers, and embedded sequences
    without requiring any explicit guards.
    ...
    -----
    gff_files : list[str]
        Per-sample Prokka GFF3 files. Files are read individually; they do not need
...
def compute_cluster_metadata(clusters, annotations):
    ...
    for rep, sample_members in clusters.items():
        all_members = [m for members in sample_members.values() for m in members]
...
def make_presence_absence_matrix(clusters, all_samples, canonical_names, outputfilename):
    """
    ...
    outputfilename : str
        Tab-separated file:
          ...
          - Columns : sample names, sorted lexicographically.
          - Values  : 1 if the cluster is present in that sample, 0 otherwise.
    """
    pa = {
        canonical_names[rep]: {s: 1 for s in sample_members}
        for rep, sample_members in clusters.items()
    }
...
def make_gene_presence_absence_csv(
    clusters, all_samples, cluster_meta, canonical_names, non_unique_genes, outputfilename
):
    """
    ...
    clusters : dict
        Output of parse_clusters().
    all_samples : list[str]
        Sorted list of all sample names (defines column order).
    cluster_meta : dict
    ...
    outputfilename : str
        ...
        Gene, Non-unique Gene name, Annotation, <sample_1>, <sample_2>, ...
    """
    rows = []
    ...
    for rep in sorted(clusters.keys()):
        ...
        for sample in all_samples:
            row[sample] = "\t".join(clusters[rep].get(sample, []))

        rows.append(row)

    cols = ["Gene", "Non-unique Gene name", "Annotation"] + list(all_samples)
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "canonical gene names as rows (index label 'Gene'), sample names as columns, "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "One or more per-sample Prokka GFF3 files (do not need to be concatenated). "
    ...
    clusters, all_samples = parse_clusters(args.mmseqs_clusters)
    ...
    make_presence_absence_matrix(clusters, all_samples, canonical_names, args.output)

    if args.gff:
        make_gene_presence_absence_csv(
            clusters, all_samples, cluster_meta, canonical_names, non_unique_genes, args.output_csv
        )
