...
def add_rna(df, genome_list, rna_folder):
    ...
    for genome in genome_list:
        rrna_file = os.path.join(
            rna_folder, "{}_rRNAs.out".format(genome)
        )
        ...
        with open(gunc_file, "r") as gunc_in:
            for line in gunc_in:
                ...
                try:
                    ...
                    logging.info(
                        "Genome {} failed GUNC and is not present in the genomes directory".format(
                            acc
                        )
                    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Creates a metadata table for all new genomes to be added "
        "to the catalog"
    )
    parser.add_argument(
        "-c",
